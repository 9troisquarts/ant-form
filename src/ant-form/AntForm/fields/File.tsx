...
export const renderFile = (file, { remove, showDate = false, index = 0 }, props = {}) => {
  ...
  const items = [
    ...
    (!isAttached || file.canDestroy) && remove && {
      key: 'remove',
      label: 'Supprimer',
      onClick: () => remove(file),
      danger: true,
  ...
  return (
    <span key={isAttached ? file.id : index} className={`ant-form-file-input-file-item lighten-background`} {...props}>
      <Space>
        ...
        <span className={`ant-form-file-input-file-item--title main-color`}>
          {(file?.name || file?.filename) ? truncate(file.name || file.filename, { length: 30, omission: '...' }) : null}
...
const FileInput: React.FC<IProps> = props => {
  ...
  const [internalValue, setInternalValue] = useState(Array.isArray(props.value || []) ? (props.value || []) : [props.value]);
  ...
  if (readOnly && ((multiple && (defaultFileList || []).length === 0) || internalValue.length === 0)) {
    return (
      <div className="is-muted">
        {emptyText || config?.file?.emptyText || 'No file'}
      </div>
    )
  }
    ...
    <>
      ...
      <Space direction="vertical" style={{ width: "100%" }}>
        {!readOnly && (internalValue.length === 0 || multiple) && (
          <>
            <Button onClick={onAddClick} icon={<UploadOutlined />} {...(addButtonProps || {})} className={`${addButtonProps?.className || ''} ant-form-file-input-add-button tenant-border lighten-background`}>
        ...
        {fileList && (
          <div>
            <Space wrap>
              {(fileList || []).map((f, i) => renderFile(
                f,
                { remove: onRemove, index: i, showDate },
                {}
              ))}
            </Space>
          </div>
