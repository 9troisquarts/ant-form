...
export const extractProxyFields = (schema: AntSchema) =>
  flattenDeep(schema).filter((field: FieldItemType) => field && field.proxy);

export const conditionnedFields = (schema: AntSchema) =>
  flattenDeep(schema).filter((field) => field && field.name && !!field?.condition);

export const extractDefaultConditionnedFields = (schema: AntSchema) =>
  conditionnedFields(schema).reduce((acc, field) => {
    acc[field.name] = {
...
export const fieldIsInactive = (inactiveFields) => (item: FieldItemType) =>
  !item?.name || !inactiveFields.includes(item.name);
